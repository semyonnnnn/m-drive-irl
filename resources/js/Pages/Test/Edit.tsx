...
export default function Edit({ auth, test }: EditProps) {
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        questions: resolveInitialQuestions(),
    });

    const [titleState, setTitleState] = useState(test.title || '');
            ...
            header={
                ...
                >
                    ...
                    <div className="flex items-center space-x-2 text-[10px] text-zinc-300 font-bold uppercase tracking-wider">
                        <span className="h-2 w-2 rounded-xs bg-amber-400 animate-pulse duration-3000" />
                        <span>редактирование_теста</span>
                    </div>
                </div>
            }
        >
            <Head title={`Редактирование теста: ${test.title}`} />
            ...
            <form onSubmit={handleSubmit} className="w-full p-4 sm:p-6 font-mono flex flex-col justify-start items-start bg-zinc-400 gap-6">
                ...
                >
                    ...
                    <div className="mb-4">
                        ...
                        />
                        {errors.title && <p className="text-red-700 text-[10px] font-bold mt-1 uppercase">[ ОШИБКА ]: {errors.title}</p>}
                    </div>
                    ...
                    <div className="mb-6">
                        ...
                        />
                        {errors.description && <p className="text-red-700 text-[10px] font-bold mt-1 uppercase">[ ОШИБКА ]: {errors.description}</p>}
                    </div>
                ...
                {data.questions.length > 0 && (
                    ...
                    >
                        ...
                        <div className="bg-zinc-200/90 p-4 border-2 border-zinc-400 clip-corner mb-2 space-y-3">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full bg-zinc-100 text-zinc-950 placeholder-zinc-500 text-xs px-3 py-2 font-bold border-2 border-zinc-400 outline-hidden focus:border-amber-600 uppercase tracking-wider clip-corner"
                                />
                                {errors.title && <p className="text-red-700 text-[9px] font-bold mt-1 uppercase">[ ОШИБКА ]: {errors.title}</p>}
                            </div>
                            <div>
                                <label className="block text-[9px] font-black text-zinc-600 uppercase mb-1">
                                    Описание и Инструкции
                                ...
                                />
                                {errors.description && <p className="text-red-700 text-[9px] font-bold mt-1 uppercase">[ ОШИБКА ]: {errors.description}</p>}
                            </div>
                        </div>
                        ...
                        <div className="flex gap-4 justify-end pb-4 mb-2 border-b-2 border-zinc-400">
                            ...
                            >
                                {processing ? 'ОБНОВЛЕНИЕ...' : '// ОБНОВИТЬ_ТЕСТ'}
                            </button>
                        </div>

                        <ul className="space-y-4">
                            {data.questions.map((q, idx) => {
                                const questionTextError = errors[`questions.${idx}.text` as keyof typeof errors];
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        {questionTextError && (
                                            <p className="text-red-700 text-[10px] font-bold mb-3 uppercase">
                                                [ ОШИБКА ]: {String(questionTextError)}
                                            </p>
                                        )}

                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-3 mt-2 border-t border-zinc-300">
                                            {q.options.map((opt, oIdx) => {
                                                const optionTextError = errors[`questions.${idx}.options.${oIdx}.text` as keyof typeof errors];
                                                ...
                                                return (
                                                    <div key={opt.id} className="flex flex-col gap-1">
                                                        ...
                                                        {optionTextError && (
                                                            <p className="text-red-700 text-[9px] font-bold uppercase">
                                                                [ ОШИБКА ]: {String(optionTextError)}
                                                            </p>
                                                        )}
                                                    </div>
                                                );
